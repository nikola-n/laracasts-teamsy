...
<div class="{{$attributes->get('class')}}">
    <label for="{{$attributes->wire('model')->value()}}"
    ...
    <div class="mt-1 relative rounded-md shadow-sm">
        ...
        />
        @error($attributes->wire('model')->value())
        <div wire:key="error_svg_{{$attributes->wire('model')->value()}}"
    ...
    @error($attributes->wire('model')->value())
    <p wire:key="error_{{$attributes->wire('model')->value()}}" class="mt-2 text-sm text-red-600" id="email-error">{{$message}}
    </p>
    @enderror
</div>
